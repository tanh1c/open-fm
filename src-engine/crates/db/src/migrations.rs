...
pub fn all_migrations() -> Migrations<'static> {
    Migrations::new(vec![
        ...
        // V24: Persist custom grid-slot tactic assignments per team
        M::up(include_str!("sql/v024_team_custom_tactic_slots.sql")),
        // V25: Persist saved tactic presets per team
        M::up(include_str!("sql/v025_team_tactic_presets.sql")),
        // V26: Persist completed transfer metadata per player
        M::up(include_str!("sql/v026_transfer_completion.sql")),
        // V27: Add competitions-first schema and fixture indexes
        M::up(include_str!("sql/v027_competitions.sql")),
        // V28: Persist season honours (champions + awards) and all-time records
        M::up(include_str!("sql/v028_season_honours.sql")),
        // V29: Persist retired players (Hall of Fame) for long-term saves
        M::up(include_str!("sql/v029_retired_players.sql")),
        // V30: Knockout stage metadata for cup + continental fixtures
        M::up(include_str!("sql/v030_fixture_knockout_stage.sql")),
        // V31: Team tactical familiarity
        M::up(include_str!("sql/v031_team_tactical_familiarity.sql")),
        // V32: Persist team tactical instructions
        M::up(include_str!("sql/v032_team_tactical_instructions.sql")),
        // V33: Persist player squad/shirt numbers
        M::up(include_str!("sql/v033_player_squad_number.sql")),
        // V34: Persist player depth-chart tier (Substitute vs Reserve)
        M::up(include_str!("sql/v034_player_squad_tier.sql")),
        // V35: Persist imported per-position player ratings
        M::up(include_str!("sql/v035_player_position_ratings.sql")),
        // V36: Persist World Cup neutral venue and group metadata
        M::up(include_str!("sql/v036_fixture_world_cup_metadata.sql")),
    ])
}
...
mod tests {
    ...
    fn test_apply_migrations_to_empty_db() {
        ...
        assert!(
            tables.contains(&"competition_teams".to_string()),
            "missing competition_teams"
        );
