...
use crate::save_index::SaveEntry;
...
impl SaveManager {
    ...
    /// List all save entries (most-recently-played first).
...
mod tests {
    ...
    #[test]
    fn test_app_db_persists_saves_across_reinit() {
        let dir = tempfile::tempdir().unwrap();
        let saves_dir = dir.path().join("saves");
...

        {
            ...
            sm.create_save(&game, "Persisted Career").unwrap();
        }
        ...
        assert_eq!(saves[0].name, "Persisted Career");
    }
    ...
    fn test_save_game_updates_existing() {
        ...
        let old_last_played = sm.list_saves()[0].last_played_at.clone();
        ...
        let saves = sm.list_saves();
        assert_eq!(saves.len(), 1);
        // last_played_at should be updated when we save again
        assert_ne!(saves[0].last_played_at, old_last_played);
