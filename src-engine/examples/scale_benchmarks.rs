...
fn main() {
    let league_counts = std::env::args()
        .skip(1)
        .map(|value| value.parse::<usize>().expect("league count must be numeric"))
        .collect::<Vec<_>>();
    ...
    println!("leagues,teams,players,competitions,fixtures,generation_ms,day_ms,week_ms,month_ms,save_ms,load_ms");
...
fn generate_scale_game(leagues: usize, season_start: chrono::DateTime<Utc>) -> Game {
    ...
    for league_index in 0..leagues {
        let country = format!("Benchmark Nation {:02}", league_index + 1);
        for team_index in 0..TEAMS_PER_LEAGUE {
            ...
            for player_index in 0..PLAYERS_PER_TEAM {
                let mut player = generator::generate_youth_academy_recruit(&teams.last().unwrap(), None);
    ...
    let mut game = Game::new(GameClock::new(season_start), manager, teams, players, staff, vec![]);
    let user_country = game.teams[0].country.clone();
    game.competitions = schedule::generate_domestic_competitions_by_country(&game.teams, 2026, season_start);
