  ...
  return (
    <div data-testid="squad-template-layout" className="flex flex-col gap-4 min-h-max max-w-[1600px] mx-auto">
      ...
      <div className={showTacticsCard ? "grid grid-cols-1 xl:grid-cols-[300px_minmax(600px,_1fr)_340px] gap-4" : "grid grid-cols-1 xl:grid-cols-[minmax(760px,_1fr)_340px] gap-4"}>
          ...
          <div className="p-3 border-b border-app-border/50 flex items-center justify-between gap-4 flex-wrap">
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="relative">
                <button type="button" onClick={() => setShowFilterPopover((current) => !current)} className="flex items-center gap-2 px-3 py-1.5 bg-app-bg border border-app-border rounded text-[11px] cursor-pointer hover:border-app-border/80 text-app-text-muted hover:text-white">
                  <span>Quick Filter</span>
                  <ChevronDown className="w-3.5 h-3.5 text-app-text-muted" />
                </button>
                {showFilterPopover ? (
                  <div className="absolute right-0 top-full mt-2 z-30 w-44 rounded-lg border border-app-border bg-app-card shadow-xl p-2">
                    {[
                      ["all", "All Players"],
                      ["xi", "Starting XI"],
                      ["bench", "Substitutes"],
                      ...
                      <button
                        ...
                        type="button"
                        onClick={() => {
                          setStatusFilter(value as FilterScope);
                          setShowFilterPopover(false);
                        }}
                        className={`w-full rounded px-2.5 py-2 text-left text-[11px] font-medium transition-colors ${statusFilter === value ? "bg-app-green/10 text-app-green" : "text-app-text-muted hover:bg-white/5 hover:text-white"}`}
                      >
                        {label}
                      </button>
                    ))}
        ...
        <TemplateCard className="flex flex-col h-[600px] min-h-0 overflow-hidden relative">
          <div className="min-h-0 flex-1 overflow-y-auto p-4 pr-3 scrollbar-thin scrollbar-track-transparent scrollbar-thumb-app-border">
      ...
      <div data-testid="squad-template-sidebar" className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-[240px_minmax(400px,_1fr)_minmax(200px,_1fr)_280px] gap-4 mt-2">
        ...
        <TemplateCard className="flex flex-col p-4">
          ...
          <SubBenchList players={substitutes} fallbackCount={7} startNumber={12} onSelect={selectPlayer} />
        ...
        <TemplateCard className="flex flex-col p-4">
          ...
          <SubBenchList players={reserves} fallbackCount={3} startNumber={19} onSelect={selectPlayer} />
          ...
          <div className="grid grid-cols-2 gap-4 h-full">
            <div className="flex flex-col">
              ...
              <div className="flex flex-col gap-2 text-[10px]">
                ...
                <div className="flex justify-between"><span className="text-app-text-muted">Home-Grown Players</span><span className="font-medium text-white">Not tracked</span></div>
            ...
            <div className="flex flex-col gap-4">
              <div className="flex flex-col">
                ...
                <div className="flex flex-col gap-1.5 text-[9px] text-app-text-muted border-b border-app-border pb-3">
                  <RegistrationRow label="Domestic (Max 25)" value={`${domesticRegistrationCount} / 25`} ok={roster.length <= 25} />
                  <RegistrationRow label="UEFA Squad" value="Not tracked" ok={null} />
                  <RegistrationRow label="Home-Grown" value="Not tracked" ok={null} />
                </div>
              </div>
  ...
  return (
    <ContextMenu items={contextItems} key={player.id}>
      <tr onClick={() => selectPlayer(player)} className={selected ? "border-b border-app-border/20 last:border-0 bg-app-green/10 hover:bg-app-green/15 transition-colors cursor-pointer" : "border-b border-app-border/20 last:border-0 hover:bg-white/5 transition-colors cursor-pointer"}>
        ...
        <td className="py-2.5 text-center"><span className={ratingClass(ovr)}>{ovr}</span></td>
        <td className="py-2.5 pr-4 text-center"><span className={ratingClass(player.potential ?? ovr)}>{player.potential ?? "—"}</span></td>
      </tr>
    </ContextMenu>
  );
}
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-2 gap-x-3 gap-y-1 text-[10px] text-app-text-muted mb-3 border-b border-app-border/50 pb-3">
        ...
        <ProfileFact label="Foot" value={formatFootedness(player.footedness)} />
        <ProfileFact label="Positions" value={getPreferredPositions(player).join(" / ")} />
        <ProfileFact label="Nationality" value={player.nationality} />
        <ProfileFact label="Expires" value={player.contract_end ?? "—"} />
        <ProfileFact label="Remaining" value={getContractYearsRemaining(player.contract_end, currentDate)} />
      </div>

      {player.traits.length > 0 ? (
        <div className="mb-3 border-b border-app-border/50 pb-3">
          <div className="text-[9px] font-bold text-app-text-muted tracking-widest uppercase mb-2">TRAITS</div>
          <TraitList traits={player.traits} size="xs" max={3} />
        </div>
      ) : null}
      ...
      <div className="flex gap-2 mt-auto min-h-0 pt-2 pb-1 border-t border-app-border/30">
        ...
        <div className="flex-1 flex flex-col justify-center gap-0.5 text-[6.5px] leading-[1.05] min-w-0 py-1.5 px-2">
          ...
          <InfoRow label="Sharpness" value={<span className="text-app-text-muted font-medium">Not tracked</span>} />
...
function SubBenchList({ players, fallbackCount, startNumber, onSelect }: { players: PlayerData[]; fallbackCount: number; startNumber: number; onSelect: (player: PlayerData) => void }) {
  ...
  return (
    <div className="flex gap-2 mb-4 overflow-x-auto pb-1">
      {visible.map((player, index) => <SubBenchCard key={player.id} player={player} squadNumber={startNumber + index} onSelect={onSelect} />)}
    </div>
  );
}

function SubBenchCard({ player, squadNumber, onSelect }: { player: PlayerData; squadNumber: number; onSelect: (player: PlayerData) => void }) {
  const ovr = getPlayerOvr(player);
  return (
    <button type="button" onClick={() => onSelect(player)} className="flex flex-col items-center flex-1 min-w-[62px] bg-[#1a202a] border border-app-border rounded-lg relative overflow-hidden group hover:border-[#3b4c66] transition-colors cursor-pointer">
      ...
      <div className="w-full flex justify-center pt-3 pb-2 relative">
        ...
        <div className="w-10 h-10 border border-red-500 rounded bg-red-900/30 flex items-center justify-center z-10 text-xl font-bold shadow-sm">
          <span className="text-white drop-shadow-md">{squadNumber}</span>
        </div>
      </div>
...
function RegistrationRow({ label, value, ok }: { label: string; value: string; ok: boolean | null }) {
  return (
    <div className="flex justify-between items-center gap-2">
      <span>{label}</span>
      <div className="flex items-center gap-1.5">
        <span className="text-white">{value}</span>
        {ok === null ? <div className="w-3 h-3 flex items-center justify-center text-app-text-muted">—</div> : ok ? <Shield className="w-3 h-3 text-app-green fill-app-green/20" /> : <div className="w-3 h-3 flex items-center justify-center text-warn-500">-</div>}
      </div>
    </div>
  );
}

function formatFootedness(value?: string): string {
